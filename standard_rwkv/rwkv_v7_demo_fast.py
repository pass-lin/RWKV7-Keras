...
load(
    name="wkv7s",
    sources=["cuda/wkv7s_op.cpp", "cuda/wkv7s.cu"],
...
@MyStatic
def sample_logits(
    logits, temperature: float = 1.0, top_p: float = 1.0, top_k: int = 0
):
    ...
    if top_p < 1:
        ...
        if top_p > 0:
            ...
            if len(idx) > 0:
                probs[idx] = cutoff + (top_p - torch.sum(probs).item()) / len(
                    idx
                )
