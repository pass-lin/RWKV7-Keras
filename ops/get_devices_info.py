...
if keras.config.backend() == "torch":
    ...
    is_intel_a770 = (
        is_intel and "Intel(R) Arc(TM) A" in torch.xpu.get_device_name(0)
    )
    device = (
        get_available_device() if get_available_device() != "hip" else "cuda"
    )
    device_torch_lib = getattr(torch, device)
    if check_pytorch_version("2.4"):
        device = "cuda" if device == "cpu" else device
        autocast_custom_fwd = functools.partial(
            torch.amp.custom_fwd, device_type=device
        )
        autocast_custom_bwd = functools.partial(
    ...
    is_tf32_supported = (
        is_nvidia and torch.cuda.get_device_capability(0)[0] >= 8
    )
    ...
    raise ("This backend not support Triton kernel")
