...
# Local modules
from src.rwkv7_tokenizer import RWKVTokenizer
from src.backbone import *
from src.rwkv7_causal_lm import *
...
class RWKV_Tmix_x070(nn.Module):
    ...
    def forward(self, x, v_first=None):
        ...
        w = -F.softplus(-(self.w0 + torch.tanh(xw @ self.w1) @ self.w2)) - 0.5  # Clamp
        k = self.key(xk)
        v = self.value(xv)
...
def convert_rwkv7_checkpoints(weights_path):
    ...
    config = {
        ...
        "head_size": 64,
        "aaa_lora": weights["blocks.2.att.a1"].shape[1],
        "decay_lora": weights["blocks.2.att.w1"].shape[1],
        "mv_lora": weights["blocks.2.att.v1"].shape[1],
        "gate_lora": weights["blocks.2.att.g1"].shape[1],
    }
    global D_AAA_LORA, D_DECAY_LORA, D_MV_LORA, D_GATE_LORA
    D_AAA_LORA = weights["blocks.2.att.a1"].shape[1]
    D_DECAY_LORA = weights["blocks.2.att.w1"].shape[1]
    D_MV_LORA = weights["blocks.2.att.v1"].shape[1]
    D_GATE_LORA = weights["blocks.2.att.g1"].shape[1]
    ...
    my_backbone.get_layer("token_embedding").set_weights([weights.pop("emb.weight")])
...
def main(_):
    ...
    with open(os.path.join(FLAGS.preset, "vocab.txt"), "w", encoding="utf-8") as f:
        f.write(vocabs)
